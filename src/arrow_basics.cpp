...
arrow::Status RunMain() {
    ...
    int8_t days_raw_1[2] = {122, 12};
    int8_t days_raw_2[3] = {17, 23, 28};
    // AppendValues, as called, puts 5 values from days_raw into our Builder object.
    ARROW_RETURN_NOT_OK(int8builder.AppendValues(days_raw_1, 2));
    ARROW_RETURN_NOT_OK(int8builder.AppendValues(days_raw_2, 3));
    // We only have a Builder though, not an Arr ay -- the following code pushes out the
    ...
    /////////////////////////
    //
    // Part 2 - Creating a RecordBatch (simple table)
    //
    /////////////////////////
    ...
    /////////////////////////
    //
    // Part 3 - Creating Chunked arrays
    //
    /////////////////////////
    ...
    // Builders.
    int8_t days_raw2[5] = {6, 12, 3, 30};
    ARROW_RETURN_NOT_OK(int8builder.AppendValues(days_raw2, 4));
    std::shared_ptr<arrow::Array> days2;
    ARROW_ASSIGN_OR_RAISE(days2, int8builder.Finish());

    int8_t months_raw2[5] = {5, 4, 11, 3};
    ARROW_RETURN_NOT_OK(int8builder.AppendValues(months_raw2, 4));
    std::shared_ptr<arrow::Array> months2;
    ARROW_ASSIGN_OR_RAISE(months2, int8builder.Finish());

    int16_t years_raw2[5] = {1980, 2001, 1915, 2020};
    ARROW_RETURN_NOT_OK(int16builder.AppendValues(years_raw2, 4));
    ...
    /////////////////////////
    //
    // Part 4 - Creating Table (based on chunked arrays)
    //
    /////////////////////////
    ...
    table = arrow::Table::Make(schema, {day_chunks, month_chunks, year_chunks}, 9);
    ...
    // //
    // // Efforts to read single values from a Arrow::Array  (incomplete)
    // //
    // std::cout << years->GetScalar(0).ValueOrDie()->ToString() << std::endl;
    // std::cout << years->GetScalar(1).ValueOrDie()->ToString() << std::endl;
    // //
    ...
    ARROW_ASSIGN_OR_RAISE(outfile, arrow::io::FileOutputStream::Open("data/test_cpp.parquet"));
    ...
    ARROW_ASSIGN_OR_RAISE(
        infile, arrow::io::ReadableFile::Open("data/test_dat.parquet")
    );
